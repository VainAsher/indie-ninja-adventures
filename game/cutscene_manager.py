...
class Scene:
    ...
    on_start_event: str | None = None  # Event to emit when scene starts
    on_end_event: str | None = None  # Event to emit when scene ends
...
class CutsceneDefinition:
    ...
    scenes: list[Scene]
    skippable: bool = True
    on_complete_event: str | None = None
...
class CutsceneManager:
    ...
    def __init__(self):
        ...
        self.current_cutscene: CutsceneDefinition | None = None
        self.current_scene_index: int = 0
        self.current_scene_time: float = 0.0
        self.total_elapsed_time: float = 0.0

        # Callbacks
        self.on_cutscene_start_callback: Callable | None = None
        self.on_cutscene_complete_callback: Callable | None = None
        self.on_scene_change_callback: Callable | None = None
    ...
    def _create_default_cutscenes(self) -> dict[str, CutsceneDefinition]:
        ...
        cutscenes["scripted_defeat"] = CutsceneDefinition(
            ...
            scenes=[
                ...
                Scene(scene_type=SceneType.FADE, text="...", duration=2.0),
            ],
    ...
    def play_cutscene(self, cutscene_id: str) -> bool:
        ...
        if self.state == CutsceneState.PLAYING:
            print("[CutsceneManager] Already playing a cutscene")
            return False
    ...
    def skip_cutscene(self) -> bool:
        ...
        if not self.current_cutscene.skippable:
            print("[CutsceneManager] Cutscene is not skippable")
            return False
    ...
    def _start_current_scene(self):
        ...
        print(
            f"[CutsceneManager] Scene {self.current_scene_index + 1}/{len(self.current_cutscene.scenes)}: {scene.speaker or 'Narrator'}"
        )
    ...
    def _complete_cutscene(self):
        ...
        if self.current_cutscene.on_complete_event:
            print(
                f"[CutsceneManager] Cutscene complete event: {self.current_cutscene.on_complete_event}"
            )
