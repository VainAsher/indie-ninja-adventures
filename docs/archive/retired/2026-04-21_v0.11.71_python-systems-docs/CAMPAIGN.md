...
The campaign is a mission-based progression mode with six regions, thirty total missions, and an ability-unlock chain that gates access to later regions and harder content. The player operates from hub worlds, launches missions/trials from hub-side access points, and returns via mission exit portals. Each ability unlocks access to regions and portals that were previously inaccessible.

Terminology used in this document:

- **Hub world**: persistent safe world for a region, with NPCs and inter-hub portals.
- **Hub spawn room (world hub room)**: the start room inside a generated hub world where players spawn on entry/return.
- **Mission/trial level**: generated instance created when a mission/trial is started from a hub.
- **Mission exit portal**: objective-gated portal in a mission/trial level that returns players to a hub world.
...
```
Hub world (player spawns in hub spawn room)
  -> Player interacts with mission giver / mission board / trial access point
  ...
  -> Player dismissed, returns to hub world (hub spawn room)
