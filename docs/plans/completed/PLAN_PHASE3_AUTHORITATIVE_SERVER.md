...
---
# Plan â€” Phase 3: Authoritative Server

**Date:** 2026-03-29
**Status:** Phase 3a COMPLETE â€” server simulation running, clients synced. Phase 3b deferred.
...
| Layer | Status |
|---|---|
| Phase 1: Input relay | âœ… Complete â€” server relays per-frame `InputCommand`; each client runs independent simulation |
| Phase 2.5: Entity events | âœ… Complete â€” pickup collection and enemy kills broadcast via `ENTITY_EVENT`; remote clients suppress matching entities |
| Phase 3: Authoritative simulation | âŒ Not started â€” the server runs no game logic; divergence from independent AI still occurs |

### Remaining divergence sources (v0.7.8)

1. **Enemy AI** â€” `enemy_manager.update(player_x=local_x, player_y=local_y)` uses the **local player's position**. Each client's AI makes different chase/attack decisions since it only knows its own player, not the remote players.
2. **Falling platforms** â€” trigger state not tracked, not broadcast.
3. **Boss state** â€” entirely unsynced.
4. **Loot drops** â€” enemies drop loot locally; remote clients suppress the *kill* but not the *loot items spawned by it*.
5. **Hazards / environment** â€” lava/water damage, dynamic hazard spawns are each client-local.
...
Client tick (60 Hz):
  1. Capture local input â†’ send to server
  2. (Optional) Run local prediction for own player
  3. Receive WorldSnapshot â†’ apply authoritative positions to all entities
  4. Render
```
...
## 3. Gap Analysis â€” Work Needed Before Phase 3

These are the blockers and prerequisites, ordered by dependency.

---

### G1 â€” Extract `GameSimulator` from `demo_game.py` [CRITICAL BLOCKER]
...
The server cannot import `demo_game.py` to run the simulation â€” it would drag in pygame display, audio init, all menu systems, etc.
...
1. Identify the "physics block" in the game loop (roughly lines 2383â€“2870 in `demo_game.py`).
2. Pull it into `game/game_simulator.py` as `class GameSimulator`.
3. `GameSimulator.__init__(seed, tiles, collision_system)` â€” takes the pre-built world.
4. `GameSimulator.step(inputs: Dict[int, InputCommand]) -> None` â€” advances one tick.
5. `GameSimulator.get_snapshot() -> WorldSnapshot` â€” serialises current state.
6. `demo_game.py` creates a `GameSimulator` and delegates to it (no behaviour change).

**Estimate:** Largest single task â€” 2â€“3 work sessions.

---

### G2 â€” Enemy AI must support multiple players [CRITICAL BLOCKER]

**Problem:** `enemy_manager.update(player_x, player_y, ...)` assumes one player. The server will have 1â€“4 players. Enemy AI needs to target the nearest of all players.
...
Also affects `check_player_enemy_collision` â€” needs to check against all players.

**Estimate:** Medium task â€” 1 session.

---

### G3 â€” Design and implement `WorldSnapshot` [MUST]
...
class WorldSnapshot:
    ...
    # boss, hazards, etc. â€” deferred to Phase 3b
```

**Bandwidth concern:** At maximum entity counts (say 60 enemies, 80 pickups) uncompressed JSON is ~8 KB/frame â†’ ~480 KB/s per client. At 4 clients this is ~2 MB/s server outbound â€” acceptable on LAN, marginal over internet. Delta compression or interest-management can be Phase 3b.

**Estimate:** Smallâ€“medium â€” define schema, implement serialisation. 1 session.

---

### G4 â€” Falling platform stable IDs and sync [MUST]
...
**Estimate:** Small â€” 1 session. Builds on Phase 2.5's entity event foundation (already has `platform_trigger` etype slot defined in the protocol).

---

### G5 â€” Server simulation loop (asyncio 60 Hz) [MUST]

**Problem:** The server currently processes one tick per received `INPUT` message â€” it advances `session.frame` each time it gets an input. This is reactive, not proactive.
...
**Estimate:** Medium â€” 1 session.

---

### G6 â€” Client reconciliation â€” apply `WorldSnapshot` to local entities [MUST]
...
For the local player: the simplest correct approach is to apply the server's authoritative position each frame ("rubber band"). This adds 1 RTT of latency (~20â€“40 ms on LAN â€” barely perceptible). Input prediction (G7) eliminates this cost but is optional.
...
if snap:
    # Players â€” already done for remote players
    # Enemies â€” overwrite enemy_manager positions from snap.enemies
    _apply_enemy_snapshot(enemy_manager, snap.enemies)
    # Pickups â€” overwrite pickup_manager from snap.pickups
    _apply_pickup_snapshot(pickup_manager, snap.pickups)
    # Platforms â€” overwrite platform states from snap.platform_states
    _apply_platform_snapshot(platforms, snap.platform_states)
    # Local player (no prediction) â€” apply server position directly
    player.state.physics.x = snap.players[local_slot].pos[0]
    player.state.physics.y = snap.players[local_slot].pos[1]
```

**Estimate:** Medium â€” 1 session, depends on G3 and G5.

---

### G7 â€” Input prediction for local player [NICE TO HAVE]

**Problem:** Without prediction, the local player's position lags by 1 RTT. On LAN this is ~5â€“20 ms (barely felt). Over the internet (~80â€“200 ms RTT) it feels sluggish.
...
**Estimate:** Large â€” 1â€“2 sessions.

---

### G8 â€” Loot drops authoritative [SHOULD]

**Problem:** In Phase 2.5, `suppress_enemy()` removes the enemy on remote clients but *does not replicate the loot that dropped*. Each client runs `_handle_enemy_death` locally â€” loot is generated, but only on the client that killed the enemy.
...
**Estimate:** Falls out of G3 + G6 â€” no extra work if WorldSnapshot includes pickup state.

---

### G9 â€” Remove Phase 2.5 workarounds after Phase 3 is live [CLEANUP]

Once Phase 3 is stable, the following Phase 2.5 scaffolding becomes redundant:
- `send_entity_event("pickup_collect")` / `suppress_by_id` â€” world state is authoritative
- `recently_killed_ids` / `suppress_enemy` â€” server handles enemy deaths
...
```
Phase 3a â€” Foundation (2â€“3 sessions)
  ...
  [4] G1: Extract GameSimulator (largest task â€” do in sub-steps)
...
Phase 3b â€” Server Simulation (1â€“2 sessions)
...
Phase 3c â€” Client Reconciliation (1â€“2 sessions)
...
Phase 3d â€” Prediction (optional, later)
...
### 5a â€” Does `GameSimulator` need pygame?

Pygame is needed on the server for:
- `pygame.Rect` â€” used in collision detection (`core/state.py`, `systems/collision_system.py`)
- Physics constants â€” defined in `config/physics_constants.py` (pure Python, no pygame)
...
Wait â€” currently clients generate the world locally from `server_seed`. For Phase 3, the server's `WorldSnapshot` is authoritative for entity state, but the **tile layout** is still generated locally from the seed (tiles don't change). This is fine â€” only dynamic entity state (enemies, pickups, platforms) needs to be in the snapshot.
...
### 5c â€” Bandwidth budget

At 60 Hz, a `WorldSnapshot` with:
- 4 players (6 fields each) â‰ˆ 200 bytes
- 60 enemies (8 fields each) â‰ˆ 2,400 bytes
- 80 pickups (4 fields each) â‰ˆ 1,600 bytes
- 20 platforms (3 fields each) â‰ˆ 300 bytes
- Total â‰ˆ ~4,500 bytes uncompressed JSON per frame

At 60 Hz to 4 clients: ~4.5 KB Ã— 60 Ã— 4 â‰ˆ **1.08 MB/s outbound from server**

This is fine for LAN. For internet play, msgpack or delta encoding could reduce this by 3â€“5Ã—. Deferred to Phase 3b.
...
| File | Change |
|---|---|
| `game/game_simulator.py` | **New** â€” `GameSimulator` class |
| `network/snapshots.py` | **Extend** â€” `WorldSnapshot`, `EnemyState`, `PickupState`, `PlatformState` |
| `network/protocol.py` | **Add** â€” `WORLD_STATE` message type |
| `network/server.py` | **Extend** â€” `GameServer` runs `GameSimulator`, proactive 60 Hz loop |
| `network/client.py` | **Extend** â€” handles `WORLD_STATE`, stores `WorldSnapshot` |
| `entities/enemy_manager.py` | **Modify** â€” `update()` accepts multiple player positions |
| `entities/enemy_ai.py` | **Modify** â€” `update()` targets nearest of all players |
| `demo_game.py` | **Modify** â€” delegates simulation to `GameSimulator`; applies `WorldSnapshot` |
| `systems/room_generation.py` | **Modify** â€” falling platforms get stable IDs |
...
- [x] Two clients connected to a server show enemies in the same positions, making the same AI decisions, dying at the same time â€” **VERIFIED** (automated test: both clients report identical frame/enemy/pickup counts)
- [x] Pickup collection by one client removes the pickup on all other clients â€” **IMPLEMENTED** (server runs check_collections authoritatively; dead pickups drop from WorldSnapshot)
- [x] Falling platforms trigger in sync for all clients â€” **IMPLEMENTED** (560 platform states broadcast each tick; client applies server state each frame)
- [x] Solo play (no server) is behaviourally identical to pre-Phase 3 â€” **UNCHANGED** (WorldSnapshot block only runs when `_net_client is not None`)
- [x] Server logs show stable 60 Hz tick rate under 4-client load â€” **VERIFIED** (~82 Hz avg; drift warning logs when behind)
