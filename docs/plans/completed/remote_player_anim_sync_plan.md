...
1. **Animation out of sync** â€” `_infer_anim_state()` derives animation from velocity only, producing
...
2. **Ghost position stutter** â€” `interpolated_pos()` defaults to `tick_ms=16.67` (60 Hz) but the
...
| RC2 | `game/game_simulator.py` | 207-225 | `get_snapshot()` computes pos/vel/health only â€” discards full player state |
...
### Phase 1 â€” Protocol: add `anim_state` to `PlayerState` âœ… DONE
...
### Phase 2 â€” Shared animation resolver âœ… DONE
...
### Phase 3 â€” Phase 1/2.5 relay path âœ… DONE
...
### Phase 4 â€” RemotePlayer: use anim_state + adaptive interpolation âœ… DONE
...
### Phase 5 â€” Wire demo_game.py call sites âœ… DONE
- Pass `anim_state=_ps.anim_state` at both `apply_state()` calls (lines 1941, 2009)

### Phase 6 â€” Tag and release âœ… DONE
...
- Non-looping animation guard uses `anim_sm.finished` â€” doesn't interrupt attacks/hurt mid-play
- `compute_anim_state()` lives in new module `entities/player_render_state.py` to avoid circular imports
  (demo_game.py â†’ entities, game_simulator.py â†’ entities; both safe)
